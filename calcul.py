...
def sous():
  ...
  window.blit(pygame.font.Font(None, 150).render("______",1,(0,0,0)), (160,120+y*100))
  pygame.display.flip()
  j=a-b-c
  result = []
  n = 0 #number of letter
  ...
  while True:
    ...
    if ch == -2 and n > 0:
      del result[0]
      n -= 1
      window.blit(pygame.image.load("white.png").convert(), (0, 222+y*100))
    if ch != -2 and n < 4:
      result=[ch]+result
      n += 1
    for i in range(len(result)):
      window.blit(pygame.font.Font(None, 150).render(str(result[-i-1]),1,(0,0,0)), (440-i*60,222+y*100))
    pygame.display.flip()
  while len(result) <5:
    result=[0]+result
  finish = 0
  ...
  if finish==j:
    window.blit(pygame.image.load("white.png").convert(), (0, 222+y*100))
    window.blit(pygame.font.Font(None, 150).render(str(j),1,(0,255,0)), (250,222+y*100))
  else:
    window.blit(pygame.image.load("white.png").convert(), (0, 222+y*100))
    window.blit(pygame.font.Font(None, 150).render(str(j),1,(255,0,0)), (250,222+y*100))
  pygame.display.flip()
  menu(2)
